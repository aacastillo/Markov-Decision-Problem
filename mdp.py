...
class MDP():
    ...
    def __init__(self, num_rows, num_cols, rewards, terminals, prob_forw, reward_default=-1.0):
        """
...
def value_iteration(mdp, gamma, epsilon):
    ...
    while(not_converged):
        new_utilities = {}
        states_below_epsilon = 0
        all_states = mdp.get_states()
        for state in all_states:
            if (not mdp.is_terminal(state)):
                max_probable_util = -100000
                for action in mdp.get_actions(state): #for each action (up,down,etc) get the state prob. utility sum for the given state
                    cur_action_probable_util_sum = get_util_prob_sum(mdp, state, action, utilities)
                    max_probable_util = max(max_probable_util, cur_action_probable_util_sum) 
                new_utility = mdp.get_reward(state) + (gamma * max_probable_util)
                if (abs(new_utility - utilities[state]) < epsilon):
                    states_below_epsilon += 1
                new_utilities[state] = new_utility
            else:
                new_utilities[state] = mdp.get_reward(state)
        utilities = new_utilities
        if (states_below_epsilon == len(all_states)-num_terminal_states):
            not_converged = False

    return utilities

def get_util_prob_sum(mdp, state, action, utilities):
    sum = 0
    state_prob_dict = mdp.get_successor_probs(state, action)
    for suc_state in state_prob_dict: #Get sum of succesor state probability utility for given action
        probability = state_prob_dict[suc_state]
        sum += (probability * utilities[suc_state])
    return sum
...
def derive_policy(mdp, utility):
    ...
    for state in all_states:
        if (not mdp.is_terminal(state)):
            ...
            max_action = ''
            for action in mdp.get_actions(state): #for each action (up,down,etc) get the state prob. utility sum for the given state
                cur_action_probable_util_sum = get_util_prob_sum(mdp, state, action, utility)
                if (cur_action_probable_util_sum >= max_probable_util):
                    max_probable_util = cur_action_probable_util_sum
                    max_action = action
...
if __name__ == "__main__":
    ...
    gridworld = MDP(ROWS, COLS, REWARDS, TERMINALS, FORW_PROB, REW_DEF)  # put the correct args here or it will error out!
